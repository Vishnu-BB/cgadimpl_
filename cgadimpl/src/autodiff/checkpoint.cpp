//============================================================
// file: cgadimpl/src/autodiff/checkpoint.cpp
//============================================================
...
bool ensure_inputs_present(const std::shared_ptr<Node>& node) {
    ...
    // Helper lambda to check and recompute a parent
    auto check_parent = [&](const std::shared_ptr<Node>& parent_node) -> bool {
        if (!parent_node) return true; // Skip nulls
        
        if (parent_node->value.numel() == 0 || parent_node->value.allocated_bytes() == 0) {
            // Recursively recompute parent, whether it is a checkpoint or not
    ...
    // If checkpoint with saved inputs, use them
...
bool recompute_subgraph(const std::shared_ptr<Node>& node) {
    ...
    // Allow recomputing non-checkpoints now
    ...
    // Recompute this node
    try {
        // Restore inputs from saved_inputs ONLY if it is a checkpoint and has saved inputs
        if (node->is_checkpoint && !node->saved_inputs.empty()) {
